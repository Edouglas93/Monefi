...
namespace Sabio.Services
{
    public class LoanApplicationService : ILoanApplicationService
    {
        ...
        public Paged<LoanApplicationBorrowerBusiness> SearchPagination(string query,int pageIndex, int pageSize )
        {
            Paged<LoanApplicationBorrowerBusiness> pagedList = null;
            List<LoanApplicationBorrowerBusiness> list = null;
            int totalCount = 0;

            _data.ExecuteCmd("[dbo].[LoanApplications_Search_ByUser]", delegate (SqlParameterCollection paramCollection)
            {
            ...
            }, delegate (IDataReader reader, short set)
            {
                ...
                if (list == null)
                {
                    list = new List<LoanApplicationBorrowerBusiness>();
                }
                list.Add(loanApp);
            }
            );
            ...
            if (list != null)
            {
                pagedList = new Paged<LoanApplicationBorrowerBusiness>(list, pageIndex, pageSize, totalCount);
            }
            return pagedList;
        }
